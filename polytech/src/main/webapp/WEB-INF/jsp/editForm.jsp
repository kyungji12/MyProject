...
<body>
	<div class="container">
		<form name="insert"onsubmit='return input_check();' method="POST" action="/save.html">
			<h2>${editForm.name }님의기본 정보 수정</h2>
			<input type="hidden" name="id" id="id" value="${editForm.id}">
			
			<label	for="fname"><i class="fa fa-user"></i> 이름</label>
			<input type="text" name="name" id="name"  maxlength="20" required value="${editForm.name}" placeholder="홍길동">
			
			<label for="email"><i class="fa fa-user"></i> 직급</label>
			<input type="text" name="position" id="position" maxlength="30" required value="${editForm.position}"placeholder="사원">
						
			<label for="adr"><i	class="fa fa-address-card-o"></i> 부서</label>			
			<input type="text" name="department"id="department" maxlength="30" required value="${editForm.department}"placeholder="영업">
			
			<label	for="fname"><i class="fa fa-user"></i> 이메일</label>
			<input type="text" name="email" id="email"maxlength="50" required value="${editForm.email}" placeholder="홍길동">
			
			<label for="city"><i class="fa fa-institution"></i> 비고란</label>
			<input type="text" name="special" maxlength="255" value="${editForm.special}" placeholder="특이사항">
			
			<div style="text-align: center;">
				<input type="submit" value="수정" class="btn"> <input
					type="button" value="취소" onclick="history.go(-1)" class="btn">
			</div>
		</form>
	</div>
</body>
